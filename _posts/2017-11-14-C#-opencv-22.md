...
`if`문을 이용하여 코너점의 갯수를 파악하여 4개보다 적으면 무시하게 합니다. 화면상의 각 모서리도 하나의 덩어리이며 코너점으로 판단하게됩니다. 화면의 모서리 부분을 무시하기 위해 5개 이상의 코너점부터 검출합니다.


`c`는 검출된 하나의 물체(덩어리)이며, 그때의 `c.Total`이 그 덩어리에 포함된 코너점입니다.
